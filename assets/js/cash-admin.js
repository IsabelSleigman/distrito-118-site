(() => {
  ...
  async function loadCash() {
    ...
    const totalInHands = (movements || []).reduce((sum, item) => sum + (item.movement_type === "entry" ? Number(item.amount) : -Number(item.amount)), 0);
    ...
    document.getElementById("cashBalance").textContent = money(totalInHands);
